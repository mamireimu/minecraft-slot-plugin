...
implements Listener
{
	public HashSet<String> rolling = new HashSet<String>();
	public int inputTypeID = -1;
	public int inputAmount = 0;
	public int outputTypeID = -1;
	public String prefix = "§6≪§e§lSlot§6≫ ";

	public void onEnable()
	{
		getServer().getPluginManager().registerEvents(this, this);
		loadConfig();
		this.getLogger().info("SlotPlugin has been enabled.");
	}

	public void onDisable()
	{
		saveConfig();
		getLogger().info("SlotPlugin has been disabled.");
	}
	...
	public void loadConfig()
	{
		...
		saveConfig();
		reloadConfig();
		if(config.get("item.input.typeID")!=null)
		{
			inputTypeID = config.getInt("item.input.typeID");
		}
		if(config.get("item.input.amount")!=null)
		{
			inputAmount = config.getInt("item.input.amount");
		}
		if(config.get("item.output.typeID")!=null)
		{
			outputTypeID = config.getInt("item.output.typeID");
		}
	}

	public boolean canRemoveItem(Inventory inv, Material type, int data, int needAmount)
	{
		int amount = 0;
		for (int i = 0; i < inv.getSize(); i++)
		{
			...
			if (item != null)
			{
				if((item.getType().equals(type)) && ((data == -1) || (data == item.getData().getData())))
				{
					int take = Math.min(item.getAmount(), needAmount-amount);
					amount += take;
				}
			}
		}
	...
	public boolean removeItem(Inventory inv, Material type, int data, int amount)
	{
		if(canRemoveItem(inv, type, data, amount))
		{
			int remd = 0;
	...
	{
		if(cmd.getName().equalsIgnoreCase("slot"))
		{
	...
	public void noItemPickDelay (Item i)
	{
		new BukkitRunnable()
		{
			@Override
